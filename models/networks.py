...
class AutoEncoder(nn.Module):
    def __init__(self, input_A, input_B):
        ...
        self.layer1_2 = nn.Linear(input_B, 400, bias = False)
        self.layer2 = nn.Linear(400*2, 500)
        self.layer3 = nn.Linear(500, 200)
...
def define_AE(input_nc, output_nc, gpu_ids=[]):
    NetAE = None
    use_gpu = len(gpu_ids) > 0
    if use_gpu:
        assert (torch.cuda.is_available())
    NetAE = AutoEncoder(input_nc, output_nc)
